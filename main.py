...
def map5():
    ...
    ball.radius = 1
    ball_pos = vector(-40+jos_special, 30, 0)
    ball.pos.x = -2+jos_special
    ball.pos.y = 30
    ball_velocity = vector(7, -gravitasi, 0)
...
button_map5 = button(bind = map5, pos = scene.title_anchor, text = "MAP5")
...
##MAP5joshhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh
jos_special = 1200
...
obstacle1 = box(pos=vector(-20+jos_special, 5, 0), length=8, width=8, height=2, color=color.blue)
obstacle2 = cone(pos=vector(0+jos_special, 20, 0), radius=6, height=12, color=color.green)
obstacle3 = cylinder(pos=vector(30+jos_special, 15, 0), radius=4, height=8, color=color.orange)
obstacle4 = compound(
    [
        box(pos=vector(45+jos_special, 25, 2), length=10, width=2, height=6, color=color.magenta),
        box(pos=vector(45+jos_special, 25, -2), length=10, width=2, height=6, color=color.magenta),
    ]
)
obstacle5 = sphere(pos=vector(60+jos_special, 5, 0), radius=6, color=color.yellow)
obstacle6 = compound(
    [
        box(pos=vector(70+jos_special, 25, 2), length=10, width=2, height=6, color=color.cyan),
        box(pos=vector(70+jos_special, 25, -2), length=10, width=2, height=6, color=color.cyan),
    ]
)
obstacle7 = pyramid(pos=vector(90+jos_special, 15, 0), size=vector(8, 8, 8), color=color.purple)
obstacle8 = compound(
    [
        cone(pos=vector(105+jos_special, 25, 0), radius=6, height=12, color=color.white),
        box(pos=vector(105+jos_special, 25, 0), length=6, width=12, height=2, color=color.white),
    ]
)

ball_acceleration = vector(7, -9.8, 0)  # Gravity
...
#            ball.trail.append(pos=ball.pos)
        ...
        #    particlez = [particle for particle in particlez if particle.pos.y > ground6.pos.y + ground6.size.y / 2 + particle.radius]
            ...
            print("test")
            rate(150)
            ball_velocity += ball_acceleration * 0.01
            ball.pos = ball.pos + ball_velocity * 0.01
            print("test")
            ...
            for obstacle in obstacles:
                if  (obstacle.pos.y - obstacle.height / 2 - ball.radius) <= ball.pos.y <= (obstacle.pos.y + obstacle.height / 2 + ball.radius) and \
                    (obstacle.pos.x - obstacle.length / 2 - ball.radius) <= ball.pos.x <= (obstacle.pos.x  + obstacle.length / 2 + ball.radius):
                        ball_velocity.y = abs(ball_velocity.y)
                        print("testing2")
            ...
            if (final_obstacle.pos.y - final_obstacle.height / 2 - ball.radius) <= ball.pos.y <= (final_obstacle.pos.y + final_obstacle.height / 2 + ball.radius) and \
               (final_obstacle.pos.x - final_obstacle.length / 2 - ball.radius)  <= ball.pos.x <= (final_obstacle.pos.x + final_obstacle.length / 2 + ball.radius):

                    ball_velocity = vector(0, 0, 0)  # Stop the ball
