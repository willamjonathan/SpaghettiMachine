...
side1 = box(pos=vector(0, box_height/2 + 1, 0), length=box_length,
            width=box_thickness, height=box_width, color=color.blue)
side1.rotate(angle=radians(box_angle), axis=vector(
...
side2 = box(pos=box2_pos, length=box2_length, width=box2_thickness,
            height=box2_height, color=color.green)
...
side3 = box(pos=box3_pos, length=box3_length, width=box3_thickness,
            height=box3_height, color=color.orange)
...
bounce_box = box(pos=bounce_box_pos, length=bounce_box_length,
                 width=bounce_box_thickness, height=bounce_box_height, color=color.yellow)
...
new_bounce_box = box(pos=new_bounce_box_pos, length=new_bounce_box_length,
                     width=new_bounce_box_thickness, height=new_bounce_box_height, color=color.yellow)
...
new_bounce_box_1 = box(pos=new_bounce_box_1_pos, length=new_bounce_box_1_length,
                       width=new_bounce_box_1_thickness, height=new_bounce_box_1_height, color=color.red)
...
push_object = box(pos=push_object_pos, length=push_object_length,
                  width=push_object_width, height=push_object_height, color=color.magenta)
...
while True:
    ...
    delta_rotated1 = rotate(
        delta1, angle=radians(-box_angle), axis=vector(0, 0, 1))
    ...
    if abs(delta_rotated1.x) < (side1.length/2 - ball.radius) and abs(delta_rotated1.z) < (side1.width/2 - ball.radius):
        if 0 < delta_rotated1.y < (side1.height/2 + ball.radius):
            ...
            normal = rotate(vector(0, 1, 0), angle=radians(
                box_angle), axis=vector(0, 0, 1))
            ball_velocity = ball_velocity - 2 * \
                dot(ball_velocity, normal) * normal
    ...
    if ball.pos.y < side3.pos.y + side3.height/2 and abs(ball.pos.x - side3.pos.x) < (side3.length/2 - ball.radius) and abs(ball.pos.z - side3.pos.z) < (side3.width/2 - ball.radius):
        # Set both vertical and horizontal velocities to zero
        ball_velocity = vector(0, 0, 0)
        # Adjust the position of the ball to be just inside the box
        ball.pos.y = side3.pos.y + side3.height/2 - ball.radius
        if popup_text is None:
            popup_text = label(pos=side3.pos + vector(side3.length/2 +
                                                      2*ball_radius, 0, 0), text="Good Game", color=color.white)
    ...
    if (ball.pos.y - ball.radius <= push_object.pos.y + push_object.height/2) and \
        ...
        if ball.pos.y - ball.radius <= push_object.pos.y + push_object.height/2:
            # Apply the force of 40 N to the ball
            ball_velocity.y = abs(ball_velocity.y) + 2
